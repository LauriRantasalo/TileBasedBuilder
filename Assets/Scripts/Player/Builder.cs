...
namespace Player
{
    public class Builder : MonoBehaviour
    {
        ...
        void UpdateSelectedTiles()
        {
            ...
            for (int x = 0; x < selectedTiles.GetLength(0); x++)
            {
                for (int y = 0; y < selectedTiles.GetLength(1); y++)
                {
                    ...
                    Tile tile = chunk.tiles[(int)tileGridPos.x % WorldMain.chunkSizeX, (int)tileGridPos.y % WorldMain.chunkSizeY];
                    ...
                    chunk.tiles[(int)tileGridPos.x % WorldMain.chunkSizeX, (int)tileGridPos.y % WorldMain.chunkSizeY] = Tile.tileTypes[uiHandler.selectedMaterialIndex];//uiHandler.selectedTileType;
        ...
        private Tile[,] FindSelectedTiles(Vector2 startGridPos, Vector2 endGridPos)
        {
            ...
            for (int x = (int)smallerGridPositions.x; x <= (int)biggerGridPositions.x; x++)
            {
                for (int y = (int)smallerGridPositions.y; y <= (int)biggerGridPositions.y; y++)
                {
                    ...
                    selectedTiles[x - (int)smallerGridPositions.x, y - (int)smallerGridPositions.y] = chunk.tiles[x - (int)chunk.position.x * WorldMain.chunkSizeX, y - (int)chunk.position.y * WorldMain.chunkSizeY];
                }
            }
        ...
        {
            Vector2 chunkPos = new Vector2(Mathf.FloorToInt(gridPos.x / WorldMain.chunkSizeX), Mathf.FloorToInt(gridPos.y / WorldMain.chunkSizeY));
            return chunkPos;
        }
        /// <summary>
